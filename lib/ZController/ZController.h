...
class ZController
{
  public:
    // constructor
    ZController(StepperMotor &z_stepper_motor, uint8_t probe_pin):
    // initializer list
    ...
    void set_linear_target(float mm)
    {
      ...
      long steps = (long)(mm*_SPR/_L);  // compute target steps using
      set_target(steps);                // set the target in steps
    }

    void set_linear_vel(float mm_per_sec)
    {
      long steps_per_second = (long)(mm_per_sec*_SPR/_L);  // compute the target steps/second
  ...
  private:
    ...
    const unsigned long _L{8};      // mm/rev
};
